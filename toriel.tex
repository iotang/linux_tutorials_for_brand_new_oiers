...
$ sudo apt install emacs
		...
		\subsection{安装中文输入法}
			...
			\subsubsection{切换到 fcitx}
				...
				\begin{figure}[H]
					\centering
					\includegraphics[width=0.3\textwidth]{fig/choose_fcitx.png}
					\caption*{选择 fcitx}
				\end{figure}
...
$ sudo apt install fcitx
				...
				\begin{figure}[H]
					\centering
					\includegraphics[width=0.3\textwidth]{fig/apply_langs_settings.png}
					\caption*{应用到整个系统}
				\end{figure}
    ...
    (compile 
        (format
            ...
            (buffer-name)(file-name-sans-extension (buffer-name))
				...
				在这里可以设置括号匹配和设置 CUA 键（剪切、复制、粘贴分别是 C-x, C-c, C-v（大写的 C 代表 Ctrl，大写的 M 代表 Alt））,以及\textbf{\large 保存设置}。
...
(global-set-key [f2] 'gud-gdb)
				...
				根据需要调整，比如我就是 3 格 Tab 缩进的毒瘤。
				
			\subsubsection{使用 Emacs}
